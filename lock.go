...
type RedisLock struct {
	LockOptions                    // redis锁的配置选项
	key         string             // 唯一标识键key
	token       string             // 使用方的身份标识 token. 同一个key 可能有多个使用方. 这个就是token
	client      LockClient         // 锁与redis交互的客户端client
	runningDog  int32              //看门狗运作标识 0表示关闭 1表示启动
	stopDog     context.CancelFunc // 停止看门狗的函数
}

// 随机生成一个token.这样就能确保不同的用户token不一样.这样每次用redis锁之前.只需要一个key redisclient 以及option就可以了
